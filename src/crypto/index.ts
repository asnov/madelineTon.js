...
if (useWebCrypto) {
    ...
    (CryptoAsync.prototype as any).igeEncrypt = CryptoWebCrypto.prototype.igeEncrypt;
    (CryptoAsync.prototype as any).getCtr = CryptoWebCrypto.prototype.getCtr;
}

(CryptoAsync.prototype as any).aesCalculate = async function (msgKey, authKey, isToServer = true) {
    const toServer = isToServer ? 0 : 8;
...
(CryptoAsync.prototype as any).oldAesCalculate = async function (msgKey, authKey, isToServer = true) {
    const toServer = isToServer ? 0 : 8;
